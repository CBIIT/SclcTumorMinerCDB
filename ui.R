	        ...
	        sidebarPanel(
	          style = "height: 120vh; overflow-y: auto;", 
	            ...
	            #selectInput("xDataset", "x-Axis patient Set", choices=dataSourceChoices, selected = "nci60"),
	        	  HTML(
	        	    paste("<label class='control-label' for='xDataset'>Horizontal Axis patient Set</label>","<select hidden id='xDataset'>",options,"</select>")
	        	  ),  ## hidden for now 
	        	  uiOutput("xPrefixUi"),
	            ...
	            #selectInput("yDataset", "y-Axis Dataset", choices=dataSourceChoices, selected = "nci60"),
	        	  HTML(
	        	    paste("<label class='control-label' for='yDataset' id='lyd'>Vertical Axis patient Set</label>","<select hidden id='yDataset'>",options,"</select>")
	        	  ),
	        	  conditionalPanel(condition="input.ts==1 || input.ts==2 || input.ts==4",
	        	  uiOutput("yPrefixUi"),
           ...
           sidebarLayout(
             sidebarPanel(
               ...
               tags$div(
                 ...
                 br(),br(),br(),
                 textInput("geneId", "Gene Identifiers: (max 5, separated by a space e.g. TP53 RB1)", "TP53 RB1"),
