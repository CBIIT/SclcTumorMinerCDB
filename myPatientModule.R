            ...
            tabPanel(
              ...
              fluidRow(
                ...
                column(
                  ...
                  h4("Please select a row on the Sample Data table and select the Find Similar Patient Button to find similar samples"), 
                ),
                column(
                  width = 12,
...
myModule <- function(input, output, session, srcContent) {
  ...
  hideTab(inputId = "innerTabs1", target = "Find Similar Patient")
  ...
  #Display Find Similar patient Tab
  observeEvent(input$find_similar_patient, {
    ...
    if (input$innerTabs1 == "Clinical Data" && !is.null(input$patient_sample_data_rows_selected)) {
      # Show the "Find Similar Patient" tab
      showTab(inputId = "innerTabs1", target = "Find Similar Patient")
      updateTabsetPanel(
        session = session,
        inputId = "innerTabs1",
        selected = "Find Similar Patient"
      )
    }
    else{
...
patient_to_patient <- reactive ({ 
    ...
    ## methylation selection .......................to complete
...
#Mutation Data
  my_mutation <- reactive ({
    ...
    if (!is.null(selected_row())) {  # Check if a row is selected
      patientID <- PatientSearchData[selected_row(), "patientID"]  # Get the patient ID of the selected row
      selected_samples <- PatientSampleData$Sample_Name[PatientSampleData$patientID == patientID]  # Get the selected patient's sample names
      mutation_cols <- c("symbol", "chromosome", "map_location", "chr_start", "chr_stop", "chr_orient", selected_samples)  # Define the columns to select from gene expression data
      ...
      mutation_cols <- c("symbol", "chromosome", "map_location", "chr_start", "chr_stop", "chr_orient", selected_samples)
      ...
      if (num_samples >= 3) {  # If there are at least 3 samples
        ...
        if (num_samples == 1) {  # If there is only 1 selected sample
          # Handle the case when num_samples == 1
          # Only default columns and selected sample
          columns_to_display <- c("symbol", "chromosome", "map_location", "chr_start", "chr_stop", "chr_orient", selected_samples)
  ...
  output$mutation_data <- DT::renderDataTable({
    datatable(
      ...
      options = list(
        columnDefs = list(
          list(visible = FALSE, targets = c(0, 2, 3, 4, 5, 6)) # Hides the first three columns
        ),
        dom = 'Bfrtip',  # Displays the column visibility button
        buttons = list('colvis'),  # Adds the column visibility button
