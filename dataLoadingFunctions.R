getTissue2SamplesMap <- function(sampleData, typeLevelSeparator = ":"){
	...
	if ("EMT" %in% colnames(sampleData)) {
		...
		stopifnot(length(intersect(names(tmp), names(tissueToSamples))) == 0)
		tissueToSamples[["EMT"]] <- emtSampleData$Name     ## New
		tissueToSamples <- c(tissueToSamples, tmp)
	}
	# ----------------------------------------------------------------------------
	# Additional phenotype-based sample sets -------------------------------------
	if ("priorTreatment" %in% colnames(sampleData)) {
	  emtSampleData <- sampleData[!is.na(sampleData$priorTreatment), , drop = FALSE]
	  tmp <- split(emtSampleData$Name, emtSampleData$priorTreatment)
	  stopifnot(length(intersect(names(tmp), names(tissueToSamples))) == 0)
	  tissueToSamples[["PriorTreatment"]] <- emtSampleData$Name     ## New
	  tissueToSamples <- c(tissueToSamples, tmp)
	}
	# ----------------------------------------------------------------------------
	# Additional phenotype-based sample sets -------------------------------------
	...
	# ----------------------------------------------------------------------------
	# Additional phenotype-based sample sets -------------------------------------
	if ("sampleType" %in% colnames(sampleData)) {
	  emtSampleData <- sampleData[!is.na(sampleData$sampleType), , drop = FALSE]
	  tmp <- split(emtSampleData$Name, emtSampleData$sampleType)
	  stopifnot(length(intersect(names(tmp), names(tissueToSamples))) == 0)
	  tissueToSamples[["SampleType"]] <- emtSampleData$Name     ## New
	  tissueToSamples <- c(tissueToSamples, tmp)
	}
	# ----------------------------------------------------------------------------
	# Additional phenotype-based sample sets -------------------------------------
	if ("biopsySite" %in% colnames(sampleData)) {
	  emtSampleData <- sampleData[!is.na(sampleData$biopsySite), , drop = FALSE]
	  tmp <- split(emtSampleData$Name, emtSampleData$biopsySite)
	  stopifnot(length(intersect(names(tmp), names(tissueToSamples))) == 0)
	  tissueToSamples[["BiopsySite"]] <- emtSampleData$Name     ## New
	  tissueToSamples <- c(tissueToSamples, tmp)
	}
	# ----------------------------------------------------------------------------
	# Additional phenotype-based sample sets -------------------------------------
	...
	if ("gender" %in% colnames(sampleData)) {
	  ...
	  tmp <- split(emtSampleData$Name, emtSampleData$gender)
	  stopifnot(length(intersect(names(tmp), names(tissueToSamples))) == 0)
	  tissueToSamples[["Gender"]] <- emtSampleData$Name     ## New
	  tissueToSamples <- c(tissueToSamples, tmp)
	}
	
	if ("sampleAutopsy" %in% colnames(sampleData)) {
	  emtSampleData <- sampleData[!is.na(sampleData$sampleAutopsy), , drop = FALSE]
	  tmp <- split(emtSampleData$Name, emtSampleData$sampleAutopsy)
	  stopifnot(length(intersect(names(tmp), names(tissueToSamples))) == 0)
	  tissueToSamples[["SampleAutopsy"]] <- emtSampleData$Name     ## New
	  tissueToSamples <- c(tissueToSamples, tmp)
	}
...
loadSourceContent <- function(srcConfig){
  ...
  if (is.null(drugFeaturePrefix)){
    ...
    # src$defaultFeatureY <- src$featurePrefixes[1]
    src$defaultFeatureY <- src$featurePrefixes[2]
  }
