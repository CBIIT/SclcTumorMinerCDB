					 	...
					 	sidebarLayout(
					 		sidebarPanel(
					 			...
					 			tags$div(
					 				...
					 				HTML(
					 				  paste("<label class='control-label' for=",ns("dataset"),">patient Set</label>","<select hidden id=",ns("dataset"),">",voptions,"</select>")
					 				),
					 				br(),br(),
					 				...
					 				textInput(ns("predIds"), "Predictor Identifiers: (Case-Sensitive, e.g. INSM1 CHGA)", "INSM1 CHGA DLL3 SEZ6 NEUROD1 POU2F3 NOTCH1 MYC"),
					 				br(),
...
regressionModels <- function(input, output, session, srcContentReactive, appConfig, oncolor) {
	...
	getFeatureDataMatrix <- function(dataSetName, dataTypes, srcContent, rmNaCols = TRUE,
		...
		for (dType in dataTypes) {
			...
			if (!is.null(responseVec)){
				# This ensures that any patient set restrictions associated with the 
	...
	# to current user selections). The first column indicates the patient, with
...
## added new parameter
		...
		shiny::validate(need(nrow(dataTab) > 0,
												 paste("ERROR: All patients have missing response or predictor data.")))
		
		# Filter patients (rows) by tissue type (if requested) ----------------------------------------
		...
		# or the range checks will not apply to the ultimately retained patients.
		# Iteration detail: the predictor data begins in column 3, 
		#  after the patient name and response value columns.
	...
	rmResponseData <- reactive({
		...
		yData <- list()
		yData$name <- colnames(dataTab)[2] # 1st column: patient name, 2nd column: response variable.
	...
	algoResults <- reactive({
		...
		## 10/26 limit to linear regression and Lasso
		## shiny::validate(need(nrow(dataTab) >= 10, "Insufficient number of patients."))
		...
		if (input$algorithm == "Linear Regression"){
		  ...
		  lmData <- dataTab[, -1, drop = FALSE] # First column has patient names
		  ...
		  shiny::validate(need(nrow(dataTab) >= 10, "Insufficient number of patients."))
			...
			# First column has patient names, second column has response data.
			...
			if (ncol(dataTab) > 2){
				...
				if (length(featuresToAdd) > 0){
					...
					# (1) Match patients between userPredData and lassoPredData (some lines
			...
			shiny::validate(need(nrow(lassoPredData) > 10, 
													 "10 or more patients (with NA-free data for candidate predictor set) are required to run LASSO"))
		...
		## 10/26
		else if (input$algorithm == "Display"){
		  rmAlgoResults$algorithm <- "Display"
		}
		else{
	  ...
	  shiny::validate(need(length(input$pcGeneSets) > 0,
		...
		if (input$algorithm == "Lasso"){
			...
			if (!is.null(rmAlgoResults$updatedInputData)){
				...
				# may drop patients with missing values for candidate predictors,
...
#			pcResults$FDR   <- signif(pcResults$FDR, 3)
	...
	# between high and low response patients.
	diffExpResultsTab <- eventReactive(input$computeDiffExp, {
		...
		# Order data matrix columns (patients) by decreasing response values.
	...
	output$data <- DT::renderDataTable({
		dat <- inputData()
		## 10/26
		if (nrow(dat) >= 10 & input$algorithm!="Display"){
		# if (nrow(dat) >= 10 ){
			rmAlgoResults <- algoResults()
			if (!is.null(rmAlgoResults$updatedInputData)){
...
##    														withSpinner(plotlyOutput(ns("heatmap"))),	
															  p("Select patient or feature name to highlight heatmap columns or rows, respectively."),br(),
	...
	output$responseDataTypeUi <- renderUI({
		...
		choices  = srcContent[[input$dataset]][["featurePrefixes"]]
		## new
		ivar = match("var",choices)
		if (!is.na(ivar)) 	choices = choices[-ivar]
		## 
